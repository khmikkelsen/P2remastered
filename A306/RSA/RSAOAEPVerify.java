...
public class RSAOAEPVerify extends RSAOAEP
{
    ...
    public RSAOAEPVerify (String signatureBase64, String messageBase64, int sLen, RSAKey sender)
            throws IOException, BadVerificationException
    {
        this.signature = Base64.getDecoder().decode(signatureBase64);
        this.sender = sender;
        this.M = Base64.getDecoder().decode(messageBase64);
        this.sLen = sLen;
        this.modBits = sender.getModulus().bitLength();
        this.EM = RSAVerify();
        verifyMessage(modBits-1);
    }
    public RSAOAEPVerify (String signatureBase64, String messageBase64, RSAKey sender)
            throws IOException, BadVerificationException
    {
        this.signature = Base64.getDecoder().decode(signatureBase64);
        this.sender = sender;
    ...
    public RSAOAEPVerify (Message message) throws IOException, BadVerificationException
    {
        this.signature = message.getSignature().getBytes();
        this.sender = message.getSenderPublicKey();
        this.M = message.getSignatureInputString().getBytes();
        this.sLen = 0;
        this.modBits = sender.getModulus().bitLength();
        this.EM = RSAVerify();
        verifyMessage(modBits-1);
